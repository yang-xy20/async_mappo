...
class MiniGridEnv(gym.Env):
    ...
    def __init__(
        ...
        # low=0, high=255, shape=(2, self.full_w, self.full_h), dtype='uint8')
        ...
        if use_agent_id:
            if use_fc_net:
                ...
                global_observation_space['vector_cnn'] = gym.spaces.Box(
                    low=0, high=255, shape=(1+self.num_agents, self.full_w, self.full_h), dtype='uint8')
        ...
        if use_stack:
            ...
            global_observation_space['stack_obs'] = gym.spaces.Box(
                low=0, high=1, shape=(c_single_stack * self.num_agents, self.full_w, self.full_h), dtype='uint8')
